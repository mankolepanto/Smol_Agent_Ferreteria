...
class RespuestaFerreteria(BaseModel):
    ...
    razonamiento: str = Field(
        description="Texto con el proceso de resolución, actualizado con las preguntas y respuestas hasta el momento"
    )
...
def answer_agent(response: dict) -> RespuestaFerreteria:
    ...
    """
    try:
        if isinstance(response['razonamiento'], (dict, list)):  # JSON en Python es dict o list
            response['razonamiento'] = json.dumps(response['razonamiento'], indent=2)  # Convierte JSON a cadena de texto legible
...
5. REGLAS IMPORTANTES:
   - Eres un asistente útil y preciso. Responde de manera clara y directa.
   - IMPORTANTE: Bajo ninguna circunstancia debes incluir etiquetas como <think>, <thought>, <reasoning> ni ningún otro tipo de anotación en la respuesta.
        Si lo haces, se considerará una respuesta incorrecta y será descartada.
   - Solo proporciona respuestas en lenguaje natural, sin formato de etiquetas, sin metadatos y sin reflexiones internas.  
...
7. Ejemplo incorrecto (NO lo hagas):  
<think> Estoy analizando la pregunta... </think>  
La respuesta es 42.  

Ejemplo correcto (Hazlo así):  
La respuesta es 42.
"""
'''
system_prompt = """\nEres un asistente especializado EXCLUSIVAMENTE en temas relacionados con el mundo de la ferretería. 
Tienes conocimientos sobre herramientas, materiales de construcción, bricolaje, reparaciones domésticas, 
pintura, cerrajería, electricidad básica, fontanería y productos relacionados con el hogar y la construcción.
